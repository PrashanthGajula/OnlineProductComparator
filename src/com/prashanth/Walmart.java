...
public class Walmart implements ProductSearch {
	
List<ProductInfo> Walmartroducts=new ArrayList<ProductInfo>();
	
	public void processMyNodes(String queryStringFormat){
			
			try{
				Parser prod_price=new Parser(queryStringFormat);
				NodeFilter prod_price_list=new HasAttributeFilter("class","price price-display");
				...
				NodeFilter prod_info_filter=new HasAttributeFilter("class","js-product-title");
				...
				{
					ProductInfo pinfo=new ProductInfo("http://www.walmart.com/search/");
//					minfo.setBrandName(prod_imageList.elementAt(i).getFirstChild().toPlainTextString());
		...
		public void searchProduct(HashMap<String, String> searchKeys) {
			...
			QueryStringFormatter formatter=new QueryStringFormatter("http://www.walmart.com/search/");
			try{
				...
				formatter.addQuery1("query",(String)searchKeys.get(ProductSearch.BRAND_NAME)+" "+(String)searchKeys.get(ProductSearch.PRODUCT_NAME));
				...
				String finalQueryString="http://www.walmart.com/search/"+formatter.getQueryString();
